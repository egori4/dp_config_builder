...
- Accepts a list of DNS profiles for update per device.
...
def run_module():
    ...
    try:
        ...
        if module.check_mode:
            ...
            if dns_profiles:
                ...
                for profile in dns_profiles:
                    ...
                    try:
                        ...
                        error_msg = (
                            f"Validation failed for profile {profile_name}: {str(e)}"
                        )
                        errors.append(error_msg)
                        logger.error(error_msg)
                        continue
                    ...
                    try:
                        ...
                        if resp.status_code in (200, 201, 204):
                            logger.info(
                                f"Successfully updated DNS profile: {profile_name}"
                            )
                            changes_made = True
                        ...
                        error_msg = (
                            f"Error updating DNS profile {profile_name}: {str(e)}"
                        )
                        errors.append(error_msg)
                        logger.error(error_msg)
            ...
            if errors:
                module.fail_json(
                    msg=f"DNS profile update completed with {len(errors)} error(s).",
                    **result,
                )
    ...
    for key, value in params.items():
        ...
        if key in ENUM_MAPS:
            ...
            if mapped_value is None:
                raise ValueError(
                    f"Invalid enum value '{value}' for {key}. Allowed: {list(ENUM_MAPS[key].keys())}"
                )
